...
abstract class ICmd<T> extends Command<T> {
  ...
  void _addFlags() {
    if (flags != null && flags.isNotEmpty) {
      flags.forEach((flag) {
        if (_optionNames.contains(flag.name))
          throw OptionOrFlagRepeatException(flag.name);
        _optionNames.add(flag.name);
        if (flag.abbr != null) {
          if (_optionNames.contains(flag.abbr) && flag.abbr != flag.name)
            throw OptionOrFlagRepeatException(flag.abbr);
          _optionNames.add(flag.abbr);
          _shortNameMap[flag.abbr] = flag.name;
        }
        argParser.addFlag(flag.name,
            abbr: flag.abbr,
            help: flag.help,
  ...
  void _addOptions() {
    if (options != null && options.isNotEmpty) {
      options.forEach((option) {
        if (_optionNames.contains(option.name))
          throw OptionOrFlagRepeatException(option.name);
        _optionNames.add(option.name);
        if (option.abbr != null) {
          if (_optionNames.contains(option.abbr) && option.abbr != option.name)
            throw OptionOrFlagRepeatException(option.abbr);
          _optionNames.add(option.abbr);
          _shortNameMap[option.abbr] = option.name;
        }
        if (option.type == OptionType.single) {
          argParser.addOption(option.name,
              abbr: option.abbr,
              help: option.help,
          ...
          argParser.addMultiOption(option.name,
              abbr: option.abbr,
              help: option.help,
              valueHelp: option.valueHelp,
  ...
  dynamic getInput(String name) {
    ...
    } else if (_shortNameMap.containsKey(name) &&
        argResults.options.contains(_shortNameMap[name])) {
  ...
  @override
  String get usage => ColorText()
      .text(_getTitle())
      .cyan('$description\n\n')
      .text(_usageWithoutDescription)
      .toString();
